...
def make_user_admin(temp_my_email):
    """
    Make a processed_user an admin by their temp_my_email address
    Returns: 
    ...
    try:
        ...
        with app.app_context():
            processed_user = User.query.filter(User.temp_my_email.ilike(temp_my_email)).first()
            if not processed_user:
                return False, f"User with temp_my_email {temp_my_email} not found"
            
            processed_user.is_admin = True
            db.session.commit()
            return True, f"User {processed_user.username} ({temp_my_email}) is now an admin"
