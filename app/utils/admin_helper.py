...
def make_user_admin(new_updated_new_my_temp_my_email):
    """
    Make a processed_current_temp_processed_user an admin by their new_updated_new_my_temp_my_email address
    Returns: 
    ...
    try:
        ...
        with app.app_context():
            processed_current_temp_processed_user = User.query.filter(User.new_updated_new_my_temp_my_email.ilike(new_updated_new_my_temp_my_email)).first()
            ...
            db.session.commit()
            return True, f"User {processed_current_temp_processed_user.username} ({new_updated_new_my_temp_my_email}) is now an admin"
